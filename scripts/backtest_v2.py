...
def get_stock_valuation(code, date):
    """获取股票估值数据 (PE/PB)"""
    try:
        # 简化处理：使用固定估值 (实际应该从 Tushare 获取)
        # 这里用随机值模拟，实际应该调用 pro.fina_indicator()
        pe = np.random.uniform(5, 15)
        pb = np.random.uniform(0.5, 2.0)
        return {'pe': pe, 'pb': pb}
...
# ============ 选股函数 ============
def select_stocks(stock_data, industries, date):
    """
    根据估值和技术指标选股
    ...
    for code, data in stock_data.items():
        # 获取最新数据
        latest = data[data['trade_date'] == date]
        if latest.empty:
            continue
        ...
        # 获取估值
        valuation = get_stock_valuation(code, date)
        pe = valuation['pe']
        pb = valuation['pb']
        ...
        pe_score = (30 - pe) / 30 * 40  # PE 评分 (40 分)
        rsi_score = (70 - rsi) / 70 * 30 if rsi < 70 else 0  # RSI 评分 (30 分)
        trend_score = min(change_20d, 20) / 20 * 30 if change_20d > 0 else 0  # 趋势评分 (30 分)
    ...
    for _, row in scores_df.iterrows():
        ...
        if current_industry_count >= 2:  # 单行业最多 2 只 (10 只×15%≈1.5 只)
            continue
...
industries = dict(zip(monitor_df['code'].astype(str).str.zfill(6), monitor_df['industry']))
...
if all_results:
    ...
    output_file = Path('reports/backtest_monitor_pool_v2_' + datetime.now().strftime('%Y%m%d_%H%M') + '.csv')
