...
FILTERS = {
    ...
    # 巴菲特财务指标（近 5 年）
    'min_roic': 8,               # 最小 ROIC 8%
    'max_debt_ratio': 60,        # 最大资产负债率 60%
    'min_roe': 8,                # 最小 ROE 8%
    'min_fcf': 0,                # 最小自由现金流 0 (要求为正)
}
...
def calculate_score(row: pd.Series) -> float:
    ...
    # ROE 净资产收益率（15% 权重）- 越高越好
    if row.get('roe', None) and row['roe'] > 0:
        if row['roe'] >= 20:
            quality_score += 100 * 0.5  # ROE>20% 满分
        elif row['roe'] >= 10:
            quality_score += (row['roe'] / 20) * 100 * 0.5  # 10-20% 线性得分
        else:
            quality_score += (row['roe'] / 10) * 50 * 0.5  # <10% 低分
    
    # 毛利率（10% 权重）- 越高越好
    if row.get('gross_margin', None) and row['gross_margin'] > 0:
        if row['gross_margin'] >= 40:
            quality_score += 100 * 0.33  # 毛利率>40% 满分
        elif row['gross_margin'] >= 20:
            quality_score += (row['gross_margin'] / 40) * 100 * 0.33
        else:
            quality_score += (row['gross_margin'] / 20) * 50 * 0.33
    
    # 负债率（5% 权重）- 越低越好
    if row.get('debt_ratio', None) and row['debt_ratio'] > 0:
        if row['debt_ratio'] <= 30:
            quality_score += 100 * 0.17  # 负债率<30% 满分
        elif row['debt_ratio'] <= 60:
            quality_score += (1 - row['debt_ratio'] / 60) * 100 * 0.17
        else:
            quality_score += max(0, (1 - row['debt_ratio'] / 100)) * 50 * 0.17
